	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (int i = 0; i < contraptionTemplate.grid.length; i++) {
			int energizedCount = Contraption
					.simulateBeamTraversal(contraptionTemplate, contraptionTemplate.grid.length - 1, i, Vector.WEST)
					.energizedTileCount();
			maxEnergizedCount = Math.max(maxEnergizedCount, energizedCount);
		}
		// columns from SOUTH
		for (int i = 0; i < contraptionTemplate.grid.length; i++) {
			int energizedCount = Contraption.simulateBeamTraversal(contraptionTemplate, i, 0, Vector.SOUTH)
					.energizedTileCount();
			maxEnergizedCount = Math.max(maxEnergizedCount, energizedCount);
		}
		// columns from NORTH
		for (int i = 0; i < contraptionTemplate.grid.length; i++) {
			int energizedCount = Contraption
					.simulateBeamTraversal(contraptionTemplate, i, contraptionTemplate.grid[0].length, Vector.NORTH)
					.energizedTileCount();
			maxEnergizedCount = Math.max(maxEnergizedCount, energizedCount);
		}
...
class Contraption {
	...
	// traversal
	public static Contraption simulateBeamTraversal(Contraption contraption, int beamXPos, int beamYPos,
			Vector beamVector) {
